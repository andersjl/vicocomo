...
/// *into-db-expr* `] ] }`
///
/// Defines a type `pub struct Opt`*other-type*`(Option<`*other-type*`>);` in the
/// module where the macro is invoked.
///
...
///
/// In the module `::vicocomo::database`, use the macro variant
///

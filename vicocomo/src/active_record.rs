...
use crate::{DatabaseIf, DbValue, Error};
...
pub trait ActiveRecord: Sized {
    ...
    /// [`Err(Error::Other("not-available")'](../error/enum.Error.html).
    ///
    /// Not overridden by the derive macro if the model has the attribute
    /// [`vicocomo_readonly`](../derive.ActiveRecord.html#vicocomo_readonly).
    ///
    /// <b>Errors</b>
    ///
    ...
    /// Return the contents of the table corresponding to `Self` in the form
    /// `INSERT INTO `*table name*` (`*column*`, `...`) VALUES`
    /// <br>`  (`*value*`, `...`), `...`;`
    ///
    fn to_sql(db: DatabaseIf) -> Result<String, Error>;

    /// Try to load the table corresponding to `Self` from a string as
    /// returned by [`to_sql()`](#tymethod.to_sql).
    ///
    /// The table is flushed before executing `sql`.
    ///
    /// The default method does nothing and returns
    /// [`Err(Error::Other("not-available")'](../error/enum.Error.html).
    ///

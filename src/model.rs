...
/// one-to-many relationship to another (or the same) type.  The trait is
/// empty because the function names are generated by the [`BelongsTo`
/// ](../derive.BelongsTo.html) derive macro.
///
/// `Remote` is the type that `Self` belongs to.  It may be `Self`.
///
...
pub trait Delete<PkType> {
    ...
    ///
    /// The implementation should to handle any remote objects associated via
    /// [`HasMany`](trait.HasMany.html) implementations.  The implementations
    /// created by `#[derive(`[`Delete`](../derive.Delete.html)`, `[`HasMany`
    /// ](../derive.HasMany.html)`)]` ensure this.
    ///
    ...
    ///
    /// The implementation should return `Err(_)` if there are any objects
    /// associated via [`HasMany`](trait.HasMany.html) implementations.  The
    /// implementations created by `#[derive(`[`Delete`
    /// ](../derive.Delete.html)`, `[`HasMany`](../derive.HasMany.html)`)]`
    /// ensure this.
    ///
    fn delete_batch(
        db: &impl DbConn,
...
/// Report errors preventing deletion of `self`.
///
pub trait DeleteErrors {
    /// If the returned vector is not empty, [`delete()`
    /// ](trait.Delete.html#tymethod.delete) should return at least those
    /// errors.  Should *not* handle errors related to [`HasMany`
    /// ](trait.HasMany.html) associations, those should be handled by
    /// [`delete()`](trait.Delete.html#tymethod.delete) directly.
    ///
    fn errors_preventing_delete(&self, db: &impl DbConn) -> Vec<String>;
}
...
/// and another (or the same) type.  The trait is empty because the function
/// names are generated by the [`HasMany`](../derive.HasMany.html) derive
/// macro.
///
/// `Remote` is the type on the other side of the relationship.
///
/// `Name` is present only to allow several, different, implementations having
/// the same `Remote` type.
///
pub trait HasMany<Remote, Name = Remote> {}

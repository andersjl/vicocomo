...
pub fn test_no_pk(db: DatabaseIf) {
    ...
    let v = View::load(db).unwrap()[0].clone();
    let mut uv = v.clone();
    uv.count = 17;
    ...
    let old_count = NoPk::load(db).unwrap().len();
    assert!(nn.insert(db).is_ok());
    let loaded = NoPk::load(db).unwrap();
    ...
    assert!(View::insert_batch(db, &mut vb[..]).is_err());
    println!("    OK");

    println!("\nreading - - - - - - - - - - - - - - - - - - - - - - - - -\n");

    println!("load() ..");
    let found = NoPk::load(db);
    assert!(found.is_ok());
    assert_eq!(&format!("{:?}", &found.unwrap()), &all_rows_str);
    let found = View::load(db);
    assert!(found.is_ok());
    ...
    not_available!("table delete", n.clone().delete(db));
    not_available!("view delete", v.clone().delete(db));
    not_available!("table delete_batch", NoPk::delete_batch(db, &[]));
    not_available!("view delete_batch", View::delete_batch(db, &[]));
    not_available!("table find", NoPk::find(db, &()), None);
    not_available!("view find", View::find(db, &()), None);
    not_available!("table find_equal", n.clone().find_equal(db), None);
    not_available!("view find_equal", v.clone().find_equal(db), None);
    not_available!("table update", un.clone().update(db));
    not_available!("view update", uv.clone().update(db));
    not_available!(
        "table update_columns",
        n.clone().update_columns(db, &[("data", DbValue::Int(17))])
    );
    not_available!(
        "view update_columns",
        v.clone()
            .update_columns(db, &[("default_parent_id", DbValue::Int(17))])
    );
    println!("validate_exists() -> Error::Model(_) ..");
    let res = NoPk::validate_exists(db, &(), "");
    assert!(res.is_err());
    if let Error::Model(_) = res.err().unwrap() {
        let res = View::validate_exists(db, &(), "");
        assert!(res.is_err());
        if let Error::Model(_) = res.err().unwrap() {
            println!("    OK");
        } else {
    ...
    not_available!("table validate_unique", n.validate_unique(db, ""), Ok);
    not_available!("view validate_unique", v.validate_unique(db, ""), Ok);
}
